...
pub fn get_events(hmm: &Hmm, motifs: &[Vec<u8>], states: &[usize], query: &[u8]) -> Vec<HmmEvent> {
    ...
    for (motif_index, hmm_motif) in hmm.motifs.iter().enumerate() {
        state_to_hmm_motif[hmm_motif.start_state..=hmm_motif.end_state].fill(motif_index as i32);
    }
    ...
    for state_index in 0..states.len() {
        ...
        let event = match offset.div_euclid(motif_len) {
            ...
            1 => HmmEvent::Ins,
            2 => HmmEvent::Del,
            _ => handle_error_and_exit("Event decoding error".to_string()),
        };
...
pub fn get_base_match(hmm: &Hmm, state: usize) -> u8 {
    ...
    if top_indexes.len() == 1 {
        match top_indexes[0] {
            ...
            3 => b'C',
            4 => b'G',
            _ => handle_error_and_exit("Unexpected base match event".to_string()),
        }
