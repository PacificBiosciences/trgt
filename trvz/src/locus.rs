...
fn get_flanks(
    ...
    let error_msg = format!("Unable to extract: {:?}", region);
...
pub fn decode(flank_len: usize, genome: &faidx::Reader, encoding: &str) -> Result<Locus, String> {
    ...
    let region = GenomicRegion::new(&encoding).unwrap();
